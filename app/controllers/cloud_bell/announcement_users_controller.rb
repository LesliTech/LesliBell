...
module CloudBell
  class AnnouncementUsersController < ApplicationController
      before_action :set_announcement_user, only: [:show, :update, :destroy]

      # GET /announcement_users
      def index
          respond_to do |format|
              format.html {}
              format.json do
                  respond_with_successful(AnnouncementUser.index(current_user, @query))
              end
          end
      end

      # GET /announcement_users/1
      def show
          respond_to do |format|
              format.html {}
              format.json do
      ...
      # GET /announcement_users/new
      def new
      end

      # GET /announcement_users/1/edit
      def edit
      end

      # POST /announcement_users
      def create
          announcement_user = AnnouncementUser.new(announcement_user_params)
      ...
      # PATCH/PUT /announcement_users/1
      def update
          return respond_with_not_found unless @announcement_user
